...
It is a local-first code comprehension tool that builds a persistent, source-grounded
workflow model of a codebase and exposes that model in two ways:
...
The underlying map is generated by static analysis, not by an LLM. Agents can render the
slice visually, explain it, expand it, or trace a path while keeping the answer constrained
to facts present in the graph.

CodeDebrief is not a documentation generator, a bug finder, a generic graph database, or
an LLM enrichment service. The core workflow is deterministic, local, and offline. No LLM
...
Months into a project, asking a coding agent to explain a workflow often means rebuilding
that understanding from raw source. The agent searches, greps, reads selected files, and
reasons over the context it managed to collect. The result may be useful, but the process
takes time, can omit less obvious paths, and is difficult to verify as a complete view of
the workflow.

CodeDebrief builds the map before the explanation. It gives humans and agents a reusable,
...
CodeDebrief serves a faster and narrower need: inspecting the workflow encoded in the
current source while brainstorming, debugging, planning a change, or reviewing impact. Use
it to:

- visualize one focused part of a workflow on demand;
- follow decisions and internal calls across files;
- inspect the workflows affected by a file, symbol, or planned change;
...
| `codedebrief.md` | Yes | Generated inspection artifact with Mermaid flowcharts; not a substitute for maintained project documentation. |
...
No. The analyzer builds the canonical workflow model and Mermaid diagrams from source.
A coding agent can choose a bounded slice, render it, translate labels, or explain it, but
