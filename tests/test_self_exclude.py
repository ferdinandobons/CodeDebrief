"""Self-exclusion of LogicChart's own source from discovery (Stage 0).
...
``src/logicchart`` or ``tests`` directory keeps all of its files.
"""
...
from logicchart.analysis.discovery import (
    _SELF_PACKAGE_DIR,
    _self_exclude_roots,
...
def test_self_exclude_drops_own_package_in_checkout() -> None:
    with_exclude = discover_source_files(
        PROJECT_ROOT, LogicChartConfig(source_roots=["src"], self_exclude=True)
    )
    without_exclude = discover_source_files(
        PROJECT_ROOT, LogicChartConfig(source_roots=["src"], self_exclude=False)
    )

    # `src/` is entirely LogicChart's own package, so self-exclude empties it.
    assert with_exclude == []
    assert any(p.resolve().is_relative_to(_SELF_PACKAGE_DIR) for p in without_exclude)


def test_self_exclude_keeps_user_files_named_like_logicchart(tmp_path: Path) -> None:
    package = tmp_path / "src" / "logicchart"
    ...
    discovered = {
        ...
        for p in discover_source_files(
            tmp_path, LogicChartConfig(source_roots=["src", "tests"], self_exclude=True)
        )
    }

    # A coincidental src/logicchart in a user project is not the running package.
